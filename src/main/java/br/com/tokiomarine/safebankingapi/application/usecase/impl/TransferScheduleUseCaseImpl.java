...
public class TransferScheduleUseCaseImpl implements TransferScheduleUseCase {
    ...
    protected BigDecimal calculateRate(Transfer transfer) {
        ...
        if(days == 0) return BigDecimal.valueOf(3.00).add(transfer.getValue().multiply(BigDecimal.valueOf(0.025)));
        if(days >= 1 && days <= 10) return BigDecimal.valueOf(12.00);
        if(days >= 11 && days <= 20) return transfer.getValue().multiply(BigDecimal.valueOf(0.082));
        if(days >= 21 && days <= 30) return transfer.getValue().multiply(BigDecimal.valueOf(0.069));
        if(days >= 31 && days <= 40) return transfer.getValue().multiply(BigDecimal.valueOf(0.047));
        if(days >= 41 && days <= 50) return transfer.getValue().multiply(BigDecimal.valueOf(0.017));
